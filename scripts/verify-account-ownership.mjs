  ...
  try {
    ...
    for (const [uid, name] of Object.entries(CUSTOMERS)) {
      ...
      if (customerAccounts.length > 0) {
        customerAccounts.forEach((account, index) => {
          ...
          console.log(
            `      Display Name: ${account.displayName || account.accountName || "N/A"}`
          );
          console.log(
        ...
        const incorrectAccounts = customerAccounts.filter(
          (acc) =>
        ...
        if (incorrectAccounts.length > 0) {
          console.log(
            `   ❌ PROBLEM: ${incorrectAccounts.length} accounts have incorrect ownership:`
          );
          incorrectAccounts.forEach((acc) => {
            console.log(
              `      - Account ${acc.id}: customerUID=${acc.customerUID}, userId=${acc.userId}`
            );
          });
        } else {
    ...
    for (const [uid, name] of Object.entries(CUSTOMERS)) {
      ...
      if (customerSnapshot.size > 3) {
        console.log(
          `   ⚠️  Expected 3 accounts, found ${customerSnapshot.size}!`
        );

        customerSnapshot.forEach((doc, index) => {
          const acc = doc.data();
          console.log(
            `      ${index + 1}. ${acc.displayName || acc.accountType} ($${(acc.balance || 0).toLocaleString()})`
          );
        });
      } else {
        console.log(
          `   ✅ Account count is correct (${customerSnapshot.size} accounts)`
        );
      }
    }
    ...
    for (const [uid, name] of Object.entries(CUSTOMERS)) {
      ...
      if (incorrectAccounts.length > 0) {
        console.log(
          `❌ ${name} has ${incorrectAccounts.length} accounts with incorrect ownership`
        );
        hasIssues = true;
      }
      ...
      if (customerAccounts.length > 3) {
        console.log(
          `⚠️  ${name} has ${customerAccounts.length} accounts (expected 3)`
        );
        hasIssues = true;
      }
    }
