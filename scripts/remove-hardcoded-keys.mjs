...
const excludeFiles = ["remove-hardcoded-keys.mjs", "firebase-config.mjs"];
...
if (!isFirebaseAvailable()) {
  ...
  process.exit(0);
          ...
          },
          {
            ...
            regex:
              /const firebaseConfig = \{[\s\S]*?apiKey:.*?AIzaSy[^"']*["'][\s\S]*?\};/gi,
...
if (!isFirebaseAvailable()) {
  ...
  process.exit(0);
    ...
    console.log(
      `✅ Updated ${updatedCount} script files to remove hardcoded API keys`
    );
