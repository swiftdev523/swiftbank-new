  ...
  try {
    ...
    if (assignments.length > 0) {
      ...
      assignments.forEach((assignment, index) => {
        ...
        console.log(
          `   Admin: ${admin ? `${admin.firstName} ${admin.lastName} (${admin.email})` : assignment.adminId}`
        );
        console.log(
          `   Customer: ${customer ? `${customer.firstName} ${customer.lastName} (${customer.email})` : assignment.customerId}`
        );
        console.log(`   Active: ${assignment.isActive !== false}`);
        console.log(
          `   Created: ${assignment.createdAt ? new Date(assignment.createdAt.seconds * 1000).toISOString() : "Unknown"}`
        );
        console.log("");
      });
    } else {
    ...
    const unassignedCustomers = customers.filter(
      (c) =>
        ...
        !assignedCustomerIds.includes(c.uid)
    );
    ...
    const unassignedAdmins = admins.filter(
      (a) =>
        !assignedAdminIds.includes(a.id) && !assignedAdminIds.includes(a.uid)
    );
    ...
    for (const customer of customers) {
      ...
      if (hasAssignment) {
        const admin = admins.find(
          (a) =>
            a.id === hasAssignment.adminId || a.uid === hasAssignment.adminId
        );
        console.log(
          `   👩‍💼 Admin: ${admin ? `${admin.firstName} ${admin.lastName}` : hasAssignment.adminId}`
        );
      } else {
    ...
    if (unassignedCustomers.length > 0) {
      console.log(
        `⚠️  ${unassignedCustomers.length} customers need admin assignment`
      );
    }

    if (unassignedAdmins.length > 0) {
      console.log(
        `⚠️  ${unassignedAdmins.length} admins need customer assignment`
      );
    }
    ...
    if (customersWithoutAccounts.length > 0) {
      console.log(
        `⚠️  ${customersWithoutAccounts.length} customers have no accounts`
      );
    }
