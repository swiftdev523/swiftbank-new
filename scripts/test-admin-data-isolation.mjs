  ...
  try {
    ...
    for (const assignment of assignments) {
      ...
      const adminAssignments = assignments.filter(
        (a) => a.adminId === adminId && a.isActive !== false
      );
      ...
      console.log(
        `   Assigned to customers: ${assignedCustomerIds.join(", ")}`
      );
      ...
      console.log(
        `   Accounts admin can access: ${adminAccessibleAccounts.length}`
      );

      if (adminAccessibleAccounts.length > 0) {
        adminAccessibleAccounts.forEach((acc) => {
          console.log(
            `      - ${acc.displayName || acc.accountType}: $${(acc.balance || 0).toLocaleString()}`
          );
      ...
      const adminAccessibleTransactions = allTransactions.filter(
        (transaction) => {
          return (
      ...
      console.log(
        `   Transactions admin can access: ${adminAccessibleTransactions.length}`
      );
      ...
      const adminAccessibleUsers = allUsers.filter(
        (user) =>
      ...
      const otherCustomers = allUsers.filter(
        (user) =>
      ...
      if (otherCustomerAccounts.length > 0) {
        console.log(
          `   ⚠️  Admin SHOULD NOT see ${otherCustomerAccounts.length} accounts from other customers`
        );
        console.log(
          `   ✅ Data isolation working: Admin filtered out unauthorized accounts`
        );
      } else {
    ...
    if (assignments.length >= 2) {
      ...
      console.log(
        `   Admin 1: ${admin1.adminId} → Customer: ${admin1.customerId}`
      );
      console.log(
        `   Admin 2: ${admin2.adminId} → Customer: ${admin2.customerId}`
      );

      // Admin 1 should NOT see Admin 2's customer
      const admin1Assignments = assignments.filter(
        (a) => a.adminId === admin1.adminId
      );
      ...
      const admin2Assignments = assignments.filter(
        (a) => a.adminId === admin2.adminId
      );
    ...
    console.log("═══════════════════════════════");
    console.log(
      `✅ Total admin-customer assignments tested: ${assignments.length}`
    );
